...
- 后续修订：新增“我的词库”管理页，并增加词库清洗候选预览与处理动作；用户可集中筛选个人词库、批量更新学习状态、批量移出个人词条，并查看低学习价值或疑似异常词的候选原因与歌词语境；词库清洗支持持久保留、加入忽略、单个或批量删除索引词条，并显示候选总量、选中数量、影响出现次数和涉及歌曲数，删除不影响歌曲原文；词汇详情支持手动标记推荐学习或低学习价值；个人词库页承接 CSV/Anki/JSON 导出、CSV/TSV 导入和清空学习记录；设置页移除日常词库管理入口，仅保留完整备份、恢复和全局清理，并统一设置页下拉框在滚动时关闭菜单；主题样式进一步 token 化，降低暮色极简与午夜航线深色主题中的白色块残留；设置页补充低学习价值词说明，并将重复歌词统计收敛为“全部统计 / 相同行去重”两个实际生效选项；学习状态入口收敛为新词、学习中、已掌握和忽略，旧的较熟悉/仅收藏状态仅保留兼容显示。
- 后续修订：导入歌曲入口迁移为独立页面 `/song/import`，形成文件选择、手动添加、草稿列表、左侧原始导入内容只读预览、右侧标题/艺术家/歌词编辑和底部固定确认栏的一页式工作区；前端导入草稿保留原始源内容、解析初始歌词和最终编辑歌词三层状态，并提供“恢复为解析初始歌词”动作，最终创建歌曲仍复用现有异步导入接口。
- 后续修订：收敛代码侧与界面侧命名，移除旧导入弹窗组件，歌词编辑组件从 `LyricStructureDialog` 更名为 `LyricEditDialog`，歌曲库状态变量改为歌词编辑语义，并删除 Quasar 初始模板残留组件，减少未使用框架噪音。
...
- 后续修订（2026-08-19）：导入成功提示保留短暂可见后再跳转歌曲库；补充 WSL 前端运行所需的 Node 版本文件，并修正启动脚本显式加载 nvm，避免误调用 Windows Node。
- 后续修订（2026-08-19）：缩短导入成功提示等待；总词库排除操作改为“常规词汇 / 已排除词汇”，排除已在个人词库中的词条前明确提示且不自动移除；我的词库将个人词条置于清洗和数据管理之前；歌词编辑补充原始标题、艺术家和歌词恢复；导入设置新增将常规英文词自动加入个人词库；出现位置补充艺术家并可进入歌曲管理查看对应歌曲；建立首版统一语义标签组件。分词器仍只提取英文字符，非英文歌词仅警告并保留歌曲。
- 后续修订（2026-08-30）：稳定歌词 token 的索引和 lemma 归一化流程，补充词元状态与数据库迁移处理，降低词汇索引构建器与词形服务之间的耦合。
- 后续修订（2026-08-30）：将词典数据源从应用数据中拆分，增加独立的词典配置 profile 和无词典运行 profile，并同步更新后端、前端及公开文档中的数据边界说明。
- 后续修订（2026-09-03）：增加短语数据模型、匹配器、查询服务和用户短语接口；词汇详情与个人词库仅展示实际命中的歌词短语，短语出现位置与单词出现位置保持分离。
- 后续修订（2026-09-03）：清理歌词出现位置中的时间戳文本，简化短语和单词的命中强调，避免展示信息与实际歌词内容混杂。
- 后续修订（2026-09-03）：增强歌词元数据识别，支持标题、艺术家、段落标签和歌曲署名的结构化保存与前端展示；重建歌词编辑器布局和元数据对照区域，补充从原始来源恢复歌词的操作。
- 后续修订（2026-09-03）：拆分词典来源与许可证信息的接口和页面职责，更新词典来源说明、API 契约、前端服务及相关测试。
